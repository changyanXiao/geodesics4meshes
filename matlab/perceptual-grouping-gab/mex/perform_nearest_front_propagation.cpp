/*=================================================================
% perform_nearest_front_propagation_2d - perform a Fast Marching front propagation.
%
%   [D,S,Q,A,SP] = perform_front_propagation_2d(W,start_points,end_points,nb_iter_max, H, L, values);
%
...
%   'Q' is a 2D array containing the Voronoi diagram
%   'A' is a (nb_start_points+1)^2 array containing the adjacency matrix
%   'SP' is a 2*k array containing the saddle_points associated with A
...
%   Copyright (c) 2008 Sébastien Bougleux
*=================================================================*/
...
#include "perform_nearest_front_propagation.h"
#include "fheap/fib.h"
...
double* Q = NULL; // Voronoi
double* adj_mtrx = NULL; // adjacency graph (subgraph of the Voronoi dual)
...
int nb_start_points1 = 0;
int nb_end_points = 0;
fibheap_el** heap_pool = NULL;
static std::list<std::pair<int,int> > saddle_points;
...
// codage de Freeman
int v8x[9] = { 1, 1, 0, -1, -1, -1, 0, 1, 1 };
int v8y[9] = { 0, -1, -1, -1, 0, 1, 1, 1, 0 };    

bool IsSaddlePoint(int x, int y)
{
    int l = (int)Q_(x,y), ll;
    if (adj_mtrx[l+nb_start_points1*l] > 1.) return false;
    int i, j;
    bool res = false;
    double min = GW_INFINITE, d = D_(x,y);
    for (int k = 0; k < 8; k++)
    {
        i = x + v8x[k]; j = y + v8y[k];
        if ((i < 0) || (j < 0) || (i>=n) || (j>=p)) continue;
        ll = (int)Q_(i,j);
        if ((S_(i,j) == kDead) && (ll != l) && (adj_mtrx[ll+nb_start_points1*ll] < 2) && 
            (adj_mtrx[ll+nb_start_points1*l] == 0.) && (adj_mtrx[l+nb_start_points1*ll] == 0.)) //&& (D_(i,j) < min))
        {
            //min = D_(i,j);
            adj_mtrx[ll+nb_start_points1*l] = 1;
            adj_mtrx[l+nb_start_points1*ll] = 1;
            adj_mtrx[ll+nb_start_points1*ll] += 1;
            adj_mtrx[l+nb_start_points1*l] += 1;
            saddle_points.push_back(std::make_pair(i,j));
            saddle_points.push_back(std::make_pair(x,y));
            return true;
            //res = true;
        }
    }
...
double* SaddlePoints(double *sdp)
{
    if (sdp == NULL) return NULL;
    std::list<std::pair<int,int> >::iterator it = saddle_points.begin(), end = saddle_points.end();
    for(int i = 0; it != end; it++)
    {
        sdp[i] = it->first; i++; sdp[i] = it->second; i++;
    }
    return sdp;
}

int NbSaddlePoints() { return saddle_points.size(); }

void perform_nearest_front_propagation(T_callback_intert_node callback_insert_node)
{
    std::ofstream outfile("crash.txt");
    
	// create the Fibonacci heap
	struct fibheap* open_heap = fh_makeheap();
	fh_setcmp(open_heap, compare_points);
    nb_start_points1 = nb_start_points + 1;
	double h = 1.0/n;
	saddle_points.clear();
	// initialize points
	for( int i=0; i<n; ++i )
    ...
    outfile << "adj_mtrx init\n";
    memset(adj_mtrx,0,nb_start_points1*nb_start_points1*sizeof(double));
    // inialize open list
