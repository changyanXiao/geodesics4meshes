function [D,S,Q,A,NN] = perform_nn_reconstruction(W, start_points, options)
...
%   A is the adjacency matrix with the degree of the Delaunay graph vertices in
%       the diagonal. A(i,j) provides the distance from the '' of ij to i. 
%   Optional:
...
%    start_points = start_points-1;
    alpha = 0;
    [D,Q,A,NN] = perform_nearest_front_propagation_anisotropic(W, L, alpha, start_points,dmax);
...
if d==2
    if exist('perform_nearest_front_propagation')~=0
        [D,S,Q,A,NN] = perform_nearest_front_propagation(W,start_points-1,end_points-1,nb_iter_max, H, L, values);
        NN = NN+1;
    else
