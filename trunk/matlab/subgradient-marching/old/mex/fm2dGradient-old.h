...
//================================================================
{
        ...
        for(k=0; k < size; k++)
            if(S[k] == kDead)
                Gradients[point].Grad[k] = ((k==point)*W[point] + alpha*Gradients[npointX].Grad[k]+ beta*Gradients[npointY].Grad[k])
                    / (alpha + beta);
        Gradients[point].nb_dead_neighbors = 2;
    ...
    //--------------------------------------------------------------
        ...
        for(k=0; k < size; k++)
            if(S[k] == kDead)
                Gradients[point].Grad[k] = (k==point) + Gradients[npointX].Grad[k];
        Gradients[point].nb_dead_neighbors = 1;
        Gradients[npointX].nb_dead_neighbors++;
        delete_condi = (Gradients[npointX].nb_dead_neighbors == 4);
        for(s=0; s <nb_end_points; s++)
             delete_condi = delete_condi && (npointX != END_POINTS[s]);
    ...
    //--------------------------------------------------------------
       ...
       for(k=0; k < size; k++)
            if(S[k] == kDead)
                Gradients[point].Grad[k] = (k==point) + Gradients[npointY].Grad[k];
        Gradients[point].nb_dead_neighbors = 1;
        Gradients[npointY].nb_dead_neighbors++;
        delete_condi = (Gradients[npointY].nb_dead_neighbors == 4);
        for(s=0; s <nb_end_points; s++)
    ...
    //--------------------------------------------------------------
        ...
        mexErrMsgTxt("recently fixed point must have at least one parent");
    }        
};
...
//================================================================
    int k;
    for(k=0; k<size; k++){
        if( (Gradients[k].nb_dead_neighbors > 0) && (Gradients[k].nb_dead_neighbors < 4) )
