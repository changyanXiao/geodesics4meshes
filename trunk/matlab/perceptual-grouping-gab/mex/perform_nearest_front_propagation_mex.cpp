...
%
%   [D,S,Q,A] = perform_front_propagation_2d(W,start_points,end_points,nb_iter_max,H,L,values);
%
...
{ 
    ...
    if ( nlhs>=4 )
    {
        plhs[3] = mxCreateDoubleMatrix(nb_start_points+1, nb_start_points+1, mxREAL); 
        adj_mtrx = mxGetPr(plhs[3]);
    }
    else
    {
        adj_mtrx = new double[(nb_start_points+1)*(nb_start_points+1)];
    }
    // launch the propagation
	perform_nearest_front_propagation();
    if (nlhs>=5 )
    {
        plhs[4] = mxCreateDoubleMatrix(2, NbSaddlePoints(), mxREAL);
        double *SP = mxGetPr(plhs[4]);
        SP = SaddlePoints(SP);
    }
