// -------------------------------------------------------------------
// -------------------------------------------------------------------
// 2D Recursive anisotropic front propagation from a given point set
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray*prhs[] ) 
{
  ...
  // retrieve arguments
  if ((nrhs < 2) && (nrhs > 5))
      mexErrMsgTxt("[U,V] = AnisoPropagation2D(tensor_field, point_set, epsilon=1.e-10, stopping_ratio=0, [hx hy]=[1 1])");
  //------------------------------------------------------------------
  ...
  double eps = 1.e-10;
  if (nrhs > 2) eps = (double)mxGetScalar(prhs[2]);
  //------------------------------------------------------------------
  // Fourth input argument: stopping criterion
  double fmax = 0;
  if (nrhs > 3) fmax = (double)mxGetScalar(prhs[3]);
  //------------------------------------------------------------------
  // Fifth input argument: spacing and dimensions
  double hx = 1, hy = 1;
  if (nrhs > 4)
  {
     const int* dim_h = mxGetDimensions(prhs[4]);
  ...
  //==================================================================
  // Propagation
  RecursiveAnisoPropagation2D<double, int> AP(TF, nx, ny, U, V, hx, hy);
  AP.propagation(seeds, nb_seeds, eps, fmax);
  //==================================================================
  AP.resize();
  dims[0] = nx;
  dims[1] = ny;
